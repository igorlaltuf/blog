...
- Qual foi a quantidade realizada de pedidos em cada ano?
...
E para carregar os pacotes que serão usados:
...
```
O DAIL contém apenas duas funções: requests() e appeals(). A primeira faz o download dos pedidos, a segunda dos dados dos recursos. Por meio do argumento "search" é possível buscar por uma palavra-chave específica.  
...
```r
requests(search = 'all') # faz o download de todos os pedidos.

requests(search = 'ovni') # faz o download de todos os pedidos e retorna aqueles que contenham a palavra "ovni" na pergunta realizada ao poder público.

requests(search = 'ovni', answer = T) # faz o download de todos os pedidos e retorna aqueles que contenham a palavra "ovni" na resposta enviada pelo poder público.
...
```
O código acima retorna uma tabela com todos os pedidos na ordem em que foram realizados. Ao todo, foram encontrados 152 pedidos de acesso à informação sobre OVNIS de 2015 até março de 2022.
...
Ao realizar a leitura de alguns dos pedidos, a explicação para a grande quantidade de pedidos nos dois anos mencionados anteriormente deve-se ao fato de que, aparentemente, a mesma pessoa realizou uma grande quantidade de pedidos para diversas instituições, o que foi identificado por meio da similaridade da escrita nas perguntas enviadas.  


Sobre os órgãos que mais responderam pedidos sobre o assunto, apenas três deles concentraram 55% das solicitações recebidas no período: COMAER (Comando da Aeronáutica), AN (Arquivo Nacional) e CEX (Comando do Exército).
...
Por fim, quanto ao tipo de resposta, apenas em 64% delas foi concedido o acesso à informação, mesmo que parcial. 
...
Por fim, para entender melhor o conteúdo dos pedidos e o tipo de informação solicitada, seria necessário verificar o conteúdo dos pedidos - e das suas respostas - por meio de uma análise qualitativa ou com a aplicação de técnicas de análise textual (também possível via linguagem R).
